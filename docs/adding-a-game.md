...
Games register through `@roomjoy/game-sdk`. The platform owns rooms, lobby, host controls, and lifecycle; modules plug in metadata + rules.
...
4. Implement as needed:
   - `createInitialState`, `onTutorialStart`, `onRoundStart`, `onTick`, `onInput`, `onAction`
   - `onEnd` → `{ summary }` for RESULTS; or `ctx.requestEnd(summary)` when the module finishes itself
   - `cleanup` when returning to library / switching games
   - `getPrivateState(ctx, playerId)` — **only that player's secrets**
   - `getPublicState(ctx)` — safe for TV + all phones (no unrevealed answers)
5. Import the package once from `apps/server/src/index.ts` (side-effect registration).
6. Keep simulation **server-side**. Phones send intents (`game_action` / `input`); TV renders public snapshots.
...
`LOBBY` → `TUTORIAL` → `PLAYING` (game `gameSubstate` + `publicGameState`) → `RESULTS` → `LOBBY`
...
| Lens | Sees |
|------|------|
| Display (TV) | Public `RoomStatePublic` only (includes sanitized `publicGameState`) |
